...
function BomEditor({ id }: { id: string }) {
  ...
  const [header, setHeader] = useState({
    code: "", product_id: "", version: "", yield_qty: 1, status: "Active", notes: ""
  });
  ...
  const { data: existingLines } = useQuery({
    ...
    enabled: !isNew,
    queryFn: async () => {
      const { data, error } = await supabase.from("bom_lines").select("*").eq("bom_id", id).is("deleted_at", null).order("line_no");
      if (error) throw error;
      return (data ?? []) as any[];
    },
  });
  ...
  useEffect(() => {
    if (doc) setHeader({
      code: doc.code ?? "", product_id: doc.product_id ?? "", version: doc.version ?? "",
      yield_qty: doc.yield_qty ?? 1, status: doc.status ?? "Active", notes: doc.notes ?? ""
    });
  }, [doc]);

  useEffect(() => {
    if (existingLines) setLines(existingLines.map(l => ({
      id: l.id, line_no: l.line_no, item_id: l.item_id ?? "", quantity: Number(l.quantity) || 0,
      unit_cost: Number(l.unit_cost) || 0, line_total: Number(l.line_total) || 0,
    })));
  }, [existingLines]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      let bomId = id;
      const payload = {
        code: header.code, product_id: header.product_id, version: header.version,
        yield_qty: Number(header.yield_qty) || 1, status: header.status, notes: header.notes,
      };

      if (isNew) {
        const { data, error } = await supabase.from("bom_headers")
          .insert({ ...payload, tenant_id: tenant.id }).select().single();
        if (error) throw error;
        bomId = data.id;
      } else {
      ...
      for (const line of lines) {
        ...
        const linePayload = {
          tenant_id: tenant.id, bom_id: bomId, line_no: line.line_no,
          item_id: line.item_id, quantity: Number(line.quantity), unit_cost: Number(line.unit_cost),
    ...
    <div className="flex flex-col gap-4 p-4 md:p-6 max-w-5xl">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link to="/manufacturing/bom"><ArrowLeft className="h-4 w-4 mr-1" /> BOMs</Link>
        </Button>
      ...
      <Card className="p-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="grid gap-1.5">
            ...
            <Input value={header.code} onChange={e => setHeader({ ...header, code: e.target.value })} disabled={!canWrite} placeholder="BOM-001" />
          </div>
          <div className="grid gap-1.5">
            <Label>Product *</Label>
            <Select value={header.product_id || undefined} onValueChange={v => setHeader({ ...header, product_id: v })} disabled={!canWrite}>
          ...
          <div className="grid gap-1.5">
            <Label>Version</Label>
            <Input value={header.version} onChange={e => setHeader({ ...header, version: e.target.value })} disabled={!canWrite} placeholder="v1" />
          </div>
          <div className="grid gap-1.5">
            <Label>Yield Qty</Label>
            <Input type="number" step="any" value={header.yield_qty} onChange={e => setHeader({ ...header, yield_qty: Number(e.target.value) })} disabled={!canWrite} />
          </div>
          <div className="grid gap-1.5">
            <Label>Status</Label>
            <Select value={header.status} onValueChange={v => setHeader({ ...header, status: v })} disabled={!canWrite}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {["Active", "Draft", "Archived"].map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="grid gap-1.5 col-span-2 md:col-span-3">
            <Label>Notes</Label>
            <Input value={header.notes} onChange={e => setHeader({ ...header, notes: e.target.value })} disabled={!canWrite} />
          </div>
        </div>
      </Card>
      ...
      <Card className="p-0 overflow-hidden">
        <div className="flex items-center justify-between border-b px-4 py-3 bg-muted/30">
          ...
          {canWrite && <Button size="sm" variant="outline" onClick={addLine}><Plus className="h-4 w-4 mr-1" /> Add Component</Button>}
        </div>
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {lines.length === 0 && (
                <TableRow><TableCell colSpan={canWrite ? 6 : 5} className="text-center text-sm text-muted-foreground py-8">No components yet. Click "Add Component" to define the recipe.</TableCell></TableRow>
              )}
              {lines.map((line, idx) => (
                <TableRow key={idx}>
                  <TableCell className="font-mono text-xs text-muted-foreground">{line.line_no}</TableCell>
                  <TableCell>
                    <Select value={line.item_id || undefined} onValueChange={v => updateLine(idx, "item_id", v)} disabled={!canWrite}>
                  ...
                  <TableCell className="text-right">
                    <Input type="number" step="any" className="h-8 text-right" value={line.quantity}
                      onChange={e => updateLine(idx, "quantity", Number(e.target.value))} disabled={!canWrite} />
                  </TableCell>
                  <TableCell className="text-right">
                    <Input type="number" step="any" className="h-8 text-right" value={line.unit_cost}
                      onChange={e => updateLine(idx, "unit_cost", Number(e.target.value))} disabled={!canWrite} />
                  </TableCell>
      ...
      {canWrite && (
        <div className="flex justify-end gap-2">
          <Button variant="outline" asChild><Link to="/manufacturing/bom">Cancel</Link></Button>
