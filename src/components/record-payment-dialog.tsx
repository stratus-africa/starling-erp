...
export function DocumentEditor({
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const headerSchema = permissionModule === "purchasing" ? PurchaseOrderSchema : InvoiceSchema;
      const headerResult = headerSchema.safeParse(headerPayload);
      if (!headerResult.success) throw new Error(formatZodError(headerResult.error));

      let docId: string | null = isNew ? null : id;
      if (isNew) {
        const data = await insertRow(cfg.table, headerResult.data as TablesInsert<typeof cfg.table>);
        docId = data.id;
      } else {
        await updateRow(cfg.table, id, headerResult.data as TablesUpdate<typeof cfg.table>);
      }

      if (!docId) throw new Error("Document ID was not created");
      ...
      if (lines.length) {
        ...
        const lineResults = linePayload.map((line) => InvoiceLineSchema.safeParse(line));
        const invalidLine = lineResults.find((result) => !result.success);
        if (invalidLine && !invalidLine.success) throw new Error(formatZodError(invalidLine.error));
