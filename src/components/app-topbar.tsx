...
import { Bell, Moon, Search, Sun, HelpCircle, LogOut, User, Settings } from "lucide-react";
...
function useBreadcrumbs() {
  ...
  return useMemo(() => {
    ...
    return [
      { label: match.group, href: match.url },
      { label: match.title, href: match.url },
    ];
  }, [pathname]);
}
    ...
    <header className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b bg-background/80 px-3 backdrop-blur">
      ...
      <div className="ml-auto flex items-center gap-2">
        <div className="relative hidden md:block">
          ...
          />
          <kbd className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 hidden md:inline-flex h-5 select-none items-center gap-1 rounded border bg-muted px-1.5 font-mono text-[10px] text-muted-foreground">
            ⌘K
          </kbd>
        </div>
          ...
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 rounded-md pl-1 pr-2 py-1 hover:bg-muted transition-colors">
              ...
              <div className="hidden md:flex flex-col items-start leading-tight">
                ...
                <span className="text-[10px] text-muted-foreground">Tenant Admin</span>
              </div>
            </button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel>Acme Manufacturing Ltd</DropdownMenuLabel>
