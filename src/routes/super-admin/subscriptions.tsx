...
export function SubscriptionManagementPage() {
  ...
  const {
    ...
    queryKey: ["super-admin", "tenant-subscriptions", searchTerm, statusFilter, paymentFilter, planFilter],
    queryFn: async () => {
  ...
  const extendTrialMutation = useMutation({
    mutationFn: async ({ subscriptionId, days, reason }: { subscriptionId: string; days: number; reason: string }) => {
    ...
    <div className="flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Super Admin subscription actions regulate platform access, quota allocations, and feature entitlement
                tiers. Tenant accounting journals, general ledger books, and ERP customer invoices are strictly isolated
                and never modified by SaaS subscription status transitions.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <Card className="p-4 bg-card border-border/70">
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">{fmtMoney(stats.totalMmr)} / mo MRR</p>
        </Card>
        ...
        <Card className="p-4 bg-card border-border/70">
          ...
          <div className="mt-2 text-2xl font-bold text-blue-600 dark:text-blue-400">{stats.trialing}</div>
        ...
        <Card className="p-4 bg-card border-border/70">
          ...
          <div className="mt-2 text-2xl font-bold text-rose-600 dark:text-rose-400">{stats.pastDue}</div>
        ...
        <Card className="p-4 bg-card border-border/70">
          ...
          <div className="mt-2 text-2xl font-bold text-muted-foreground">{stats.pausedOrCancelled}</div>
            ...
            <div className="w-full sm:w-40">
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="trialing" className="text-xs">
                    Trialing
                  </SelectItem>
              ...
              subscriptions.map((sub) => {
                ...
                return (
                  <TableRow key={sub.id} className="hover:bg-muted/20 transition-colors">
                      ...
                      <div className="text-xs space-y-0.5">
                        ...
                        <div className="text-muted-foreground text-[11px]">
                          ...
                          <span className="text-foreground">
                            {sub.current_period_end ? dateFmt(sub.current_period_end) : "—"}
                          </span>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      {sub.is_trialing ? (
                        <div className="space-y-0.5 text-xs">
                          ...
                          <div className="text-[10px] text-muted-foreground">Ends: {dateFmt(sub.trial_ends_at)}</div>
                        </div>
                      ) : (
                    ...
                    {/* Payment Status */}
                    <TableCell>
                      <Badge className={`text-[10px] border ${paymentMeta.class}`}>{paymentMeta.label}</Badge>
                    </TableCell>
                        ...
                        {canManage && (
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-52">
                              ...
                              <DropdownMenuItem
                                onClick={() => openExtendTrialDialog(sub)}
                                className="text-xs cursor-pointer"
                              >
            ...
            <div className="flex flex-col gap-6 py-4 text-xs">
              ...
              <div className="p-4 rounded-lg bg-muted/40 border flex items-start justify-between gap-4">
                ...
                <div className="space-y-1 text-right">
                  <Badge
                    className={`text-[10px] border ${(STATUS_CONFIG[selectedSub.status] ?? STATUS_CONFIG.active).class}`}
                  >
                ...
                <div className="rounded-lg border bg-card divide-y">
                  <div className="p-2.5 flex items-center justify-between">
                    ...
                    <span className="font-semibold">
                      {selectedSub.plan_name} ({selectedSub.plan_code})
                    </span>
                  </div>
                  <div className="p-2.5 flex items-center justify-between">
                  ...
                  <div className="p-2.5 flex items-center justify-between">
                    ...
                    <Badge
                      className={`text-[10px] border ${(PAYMENT_STATUS_CONFIG[selectedSub.payment_status] ?? PAYMENT_STATUS_CONFIG.paid).class}`}
                    >
                  ...
                  {selectedSub.cancellation_reason && (
                    <div className="p-2.5 space-y-1">
                      ...
                      <p className="text-xs text-foreground bg-muted p-2 rounded">{selectedSub.cancellation_reason}</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              Reassign subscription plan for{" "}
              <span className="font-semibold text-foreground">{selectedSub?.tenant_name}</span>.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-4 py-2 text-xs">
            <div className="space-y-1.5">
              <Label className="text-xs font-semibold">
                Select Target Plan <span className="text-destructive">*</span>
              </Label>
      ...
      <Dialog open={isExtendTrialOpen} onOpenChange={setIsExtendTrialOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              Add complimentary trial days for{" "}
              <span className="font-semibold text-foreground">{selectedSub?.tenant_name}</span>.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-4 py-2 text-xs">
            <div className="space-y-1.5">
              ...
              <Select value={trialDaysToAdd.toString()} onValueChange={(val) => setTrialDaysToAdd(parseInt(val))}>
                <SelectTrigger className="text-xs h-9">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
      ...
      <Dialog open={isSuspendOpen} onOpenChange={setIsSuspendOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              Temporarily pause SaaS service for{" "}
              <span className="font-semibold text-foreground">{selectedSub?.tenant_name}</span>.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-4 py-2 text-xs">
            <div className="p-3 rounded bg-amber-500/10 border border-amber-500/20 text-xs text-amber-700 dark:text-amber-300">
              ...
              <p className="mt-0.5 text-[11px]">
                The tenant's users will be temporarily restricted from accessing premium features. Historical data and
                accounting books remain intact.
              </p>
            </div>
      ...
      <Dialog open={isReactivateOpen} onOpenChange={setIsReactivateOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              Restore full active status for{" "}
              <span className="font-semibold text-foreground">{selectedSub?.tenant_name}</span>.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="flex items-center justify-between p-3 rounded-lg border bg-muted/20">
              <div className="space-y-0.5">
                ...
                <div className="text-[11px] text-muted-foreground">
                  {cancelImmediate ? "Cancels immediately right now" : "Allows usage until current period end date"}
                </div>
              </div>
