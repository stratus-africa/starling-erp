...
const finiteNumber = (label: string) =>
  z.number({ invalid_type_error: `${label} must be a number` }).finite(`${label} must be a finite number`);
...
const percentage = (label: string) =>
  finiteNumber(label).min(0, `${label} cannot be negative`).max(100, `${label} cannot exceed 100%`);
...
export const InvoiceLineSchema = z
  ...
  .superRefine((line, ctx) => {
    const expected =
      Math.round(line.quantity * line.unit_price * (1 - line.discount_pct / 100) * (1 + line.tax_pct / 100) * 100) /
      100;
    if (Math.abs(expected - line.line_total) > 0.01) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["line_total"],
        message: "Line total does not match quantity, price, discount and tax",
      });
    }
  });
  ...
  .superRefine((payment, ctx) => {
    ...
    if (payment.customer_id && payment.supplier_id) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["supplier_id"],
        message: "Payment cannot target both customer and supplier",
      });
    }
  });
  ...
  .superRefine((line, ctx) => {
    ...
    if (line.debit === 0 && line.credit === 0) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["debit"],
        message: "A journal line must contain a debit or credit",
      });
    }
  });
